...
def find_largest_content_in_zip(zip_path):
    """
    [NEW] Đọc nội dung file lớn nhất từ zip mà không giải nén ra disk.
    Tương tự find_dumpstate_content() trong dumpstate_parser.py.
    Fix lỗi long path trên Windows Server.
    """
    try:
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            ...
            # Duyệt qua danh sách file trong zip mà không giải nén
            ...
            # Nếu tìm thấy file phù hợp, đọc nội dung
            if largest_zinfo:
...
# [DEPRECATED] Old function that extracts to disk - causes long path issues
...
def parse_pageboostd_from_string(content):
    """
    [NEW] Parse pageboostd data từ string content.
    Tương tự parse_pageboostd() nhưng nhận string thay vì file path.
    Fix lỗi long path bằng cách đọc từ memory.
    """
    results = {}
...
def parse_pageboostd(file_path):
    """
    [KEEP] Original function for backward compatibility with extracted mode.
    """
    results = {}
    pattern = re.compile(r"app\s+(\S+)\s+data_amount\s+(\d+)")
...
def collect_cycles_from_zips(folder):
    """
    [UPDATED] Thu thập cycles từ zip files mà không giải nén ra disk.
    Fix lỗi long path trên Windows Server bằng cách đọc trực tiếp từ memory.
    """
    ...
    for fname in sorted(os.listdir(folder)):
        ...
        # [NEW] Đọc trực tiếp từ memory thay vì extract ra disk
        content = find_largest_content_in_zip(fpath)
        if not content:
            continue
            
        # [NEW] Parse từ string content thay vì file path
        data = parse_pageboostd_from_string(content)
...
def diff_pageboostd(folder1, folder2, extracted=False):
    """
    [UPDATED] Compare pageboostd data between two folders.
    Fix lỗi long path bằng cách đọc trực tiếp từ memory, không cần temp folder.
    """
    ...
    if extracted:
        ...
        cycles1 = collect_cycles_from_zips(folder1)  # ✅ Không cần temp folder
        cycles2 = collect_cycles_from_zips(folder2)  # ✅ Không cần temp folder
    ...
    # [REMOVED] Không cần cleanup temp folder nữa vì không tạo temp files
...
# [DEPRECATED] Old function with temp folder cleanup - no longer needed
# def diff_pageboostd(folder1, folder2, extracted=False):
#     prefix1 = get_prefix(folder1)
#     prefix2 = get_prefix(folder2)
# 
