...
def count_crashes(dumpstate_content: str) -> int:
    """
    Đếm số lượng crash events trong dumpstate.
    Patterns to detect: ANR, FATAL EXCEPTION
    
    Args:
        dumpstate_content: Nội dung file dumpstate.txt
        
    Returns:
        int: Tổng số lượng crash events (ANR + FATAL EXCEPTION)
    """
    if not dumpstate_content:
        return 0
    ...
    # Pattern 1: ANR
    # Format: "ANR in com.sec.android.app.camera (pid 12345)"
    ANR_PATTERN = re.compile(r"ANR in (.+?)(?:\s+\(pid\s+(\d+)\))?")
    
    # Pattern 2: FATAL EXCEPTION
    # Format: "FATAL EXCEPTION: main pid 12345"
    FATAL_PATTERN = re.compile(r"FATAL EXCEPTION: (.+?)(?:\s+pid\s+(\d+))?")
    
    # Đếm số lượng ANR
    anr_matches = ANR_PATTERN.findall(dumpstate_content)
    crash_count += len(anr_matches)
    
    # Đếm số lượng FATAL EXCEPTION
    fatal_matches = FATAL_PATTERN.findall(dumpstate_content)
    crash_count += len(fatal_matches)
